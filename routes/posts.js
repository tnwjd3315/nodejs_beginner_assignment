...
router.post("/posts/", async(req,res) => {
  const {postsId, user, title, content, createdAt, password} = req.body
  // // find로 게시글 조회
  ...
  const createdPosts = await Posts.create({postsId, user, title, content, createdAt, password})
...
router.put("posts/:_id", async(req, res) => {
  ...
  //postid를 사용해서 db에서 수정하려는 post를 가져오기, 그 post에 해당하는 pw가 일치하면 
  // 객체 보내주기
  // postsId를 Object로 만들기
  await Posts.findById(ObjectId(_id), async (err, doc) => {
    if (err) {
      console.error(err)
    }
